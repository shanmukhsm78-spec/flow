...
| **Ghost** | **LIVE** | `app/public/index.html`, `app/server.js`, `scripts/wifi-off-proof.sh` | Wifi off → stuck/help/receipt OR `bash scripts/wifi-off-proof.sh` → receipt in memory.json → delete; Silence blocks Ghost (403). Proof log: `PROOF-wifi-off-20260923-130509.txt` (2026-09-23). | Invisible OS stealth; cloud Ghost; “world’s first” |
...
- **Wifi-off Ghost:** proven 2026-09-23 on founder Mac (Wi-Fi Off, public net fail, Ollama local, chat_ok, receipt + delete, Silence 403). Log: `PROOF-wifi-off-20260923-130509.txt`.
